...
public class CreateUser extends AppCompatActivity {
                    ...
                    mAuth.createUserWithEmailAndPassword(user,pword).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                        ...
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            if(task.isSuccessful()){
                                Toast toast = Toast.makeText(getApplicationContext(),"Создан Новый Пользователь",Toast.LENGTH_SHORT);
                                toast.show();
                                ...
                                Toast toast = Toast.makeText(getApplicationContext(),"Не удалось Создать Нового Пользователя",Toast.LENGTH_SHORT);
                                toast.show();
                            }
                        }
                    });
    ...
    private boolean validate_user_input(String uname, String pass){
        ...
        if(uname.contains("@") && uname.contains(".")){
            ...
            banner.setText("Неверная электронная почта");
        }
        if(pass.length()>6){
            count+=1;
        }
        else {
            banner.setText("длина пароля должна составлять не менее 6 символов");
        }
